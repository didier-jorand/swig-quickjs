  ...
  if (Getattr(n, "feature:cbases")) {
    ...
    while (it.item) {
      ...
      if (p && Getattr(p, "classtype")) {
        ...
        if (p) {
          Printv(stdout, "Warning: '", Getattr(n, "sym:name"), "' base class not found: '", it.item, "' ", Getattr(p, "kind"), " (used before declaration?)\n", NIL);
          //Swig_print_node(p);
        }
...
:  JSEmitter(JSEmitter::JavascriptCore), NULL_STR(NewString("NULL")), VETO_SET(NewString("SWIG_QuickJS_veto_set_variable")), f_wrap_cpp(NULL), f_runtime(NULL), f_header(NULL), f_init(NULL) {
}
